...
import { useSchoolFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['school-administrator', 'teacher', 'school-staff', 'student', 'school-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSchoolFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Schools',
      path: isTenantUser ? `/schools/view/${data?.id}` : '/schools',
      entity: 'school',
      service: AccessServiceEnum.PROJECT,
      icon: FiHome,
    },
    { name: 'Leaves', path: '/leaves', entity: 'leave', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
