...
import { useTeacherCount, useLeaveCount, useStudentCount } from 'lib/roq';

import { LeaveListPage as ShowCaseList } from 'pages/leaves';

function DashboardPage() {
  const { session } = useSession();

  const teacherStat = useTeacherCount();

  const leaveStat = useLeaveCount();

  const studentStat = useStudentCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Teachers',
      result: teacherStat?.data,
      isLoading: teacherStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Leaves',
      result: leaveStat?.data,
      isLoading: leaveStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Students',
      result: studentStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`leaves`} passHref legacyBehavior>
                  <Button
