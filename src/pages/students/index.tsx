...
import { useRoqClient, useStudentFindManyWithCount } from 'lib/roq';
...
export function StudentListPage(props: StudentListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['class.contains', 'section.contains'],
          ...params.filters,
        },
        'student',
      ),
  ...
  const { data, error, isLoading, mutate } = useStudentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.student.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.enrollment_date ? format(record?.enrollment_date, 'dd-MM-yyyy') : '',
    },
    { id: 'grade_level', header: 'grade_level', accessorKey: 'grade_level' },
