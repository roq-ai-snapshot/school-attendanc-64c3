...
import { useRoqClient, useStaffFindManyWithCount } from 'lib/roq';
...
export function StaffListPage(props: StaffListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['position.contains', 'department.contains'],
          ...params.filters,
        },
        'staff',
      ),
  ...
  const { data, error, isLoading, mutate } = useStaffFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.staff.delete({ where: { id } });
      await mutate();
    } catch (error) {
