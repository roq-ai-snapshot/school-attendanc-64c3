...
import { useRoqClient, useTeacherFindFirst } from 'lib/roq';
...
function TeacherEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTeacherFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TeacherInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.teacher.update({
        data: values as RoqTypes.teacher,
        where: {
          id,
        },
      });
