...
import { useRoqClient, useSchoolFindManyWithCount } from 'lib/roq';
...
export function SchoolListPage(props: SchoolListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'staff.count', 'student.count', 'teacher.count'],
  ...
  const { data, error, isLoading, mutate } = useSchoolFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.school.delete({ where: { id } });
      await mutate();
    } catch (error) {
