...
import { useRoqClient, useLeaveFindFirst } from 'lib/roq';
...
function LeaveEditPage() {
  ...
  const { data, error, isLoading, mutate } = useLeaveFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: LeaveInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.leave.update({
        data: values as RoqTypes.leave,
        where: {
          id,
        },
      });
