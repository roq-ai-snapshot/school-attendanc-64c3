...
import { useRoqClient, useLeaveFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<LeaveInterface, unknown>;
...
export function LeaveListPage(props: LeaveListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['reason.contains', 'status.contains'],
          ...params.filters,
        },
        'leave',
      ),
  ...
  const { data, error, isLoading, mutate } = useLeaveFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.leave.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: LeaveInterface) => {
    if (hasAccess('leave', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/leaves/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/leaves/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('leave', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/leaves/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('leave', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="leave" />
        </Flex>

        {hasAccess('leave', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/leaves/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
